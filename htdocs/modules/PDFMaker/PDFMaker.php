...
//      when only one template is available for module, then set it as default
    ...
    // danzi.tn@20141105-1e
        ...
        if (count($TemplateContent) > 0) {
            foreach ($TemplateContent AS $templateid => $TContent) {
                ...
                foreach ($ListViewBlock[$templateid] AS $id => $text) {
                    ...
                    $cridx = 1;
		    // danzi.tn@20140102-2
		    $tmptable = "<table id='danzi_tn'>";
                    foreach ($records as $record) {
			$tmp_str = implode("", $ListViewBlockContent[$templateid][$id][$record]);
			$tmptable .= $tmp_str;
			$log->debug("GetPreparedMPDF(templateid=".$templateid.", viewid=".$viewid.") tmp_str=".$tmp_str);
                        $replace .= $tmp_str;
		...
		if(isset($tots)) {
		    ...
		    {
			$body_html = str_ireplace('$'.$key.'$', $value, $body_html);
		    }
		}
	...
	$log->debug("Exiting GetPreparedMPDF(module=".$module.") method ...");
        return $name;
    }
	...
	$table =$xpath->query("//*[@id='danzi_tn']")->item(0);
	...
	foreach ($rows as $row) {
	    ...
	    $col0Val = $cells->item(0)->nodeValue;
	    $date_val = strtotime(trim($col0Val));
	    $date_array[$rownum] = date( "Y-m-d" , $date_val);
	    $col3Val = its4you_formatNumberFromPDF($cells->item(3)->nodeValue);
	    $col4Val = its4you_formatNumberFromPDF($cells->item(4)->nodeValue);
	    $log->debug("col 3 ".$col3Val);
	    $log->debug("col 4 ".$col4Val);
	    $ore += floatval($col3Val);
	    $km += floatval($col4Val);
	    $rownum++;
	}
	...
	$periodo = DateTime::createFromFormat('Y-m-d',$first_day)->format('d.m.Y')." - ".DateTime::createFromFormat('Y-m-d',$last_day)->format('d.m.Y');
