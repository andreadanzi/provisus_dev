...
class HelpDeskHandler extends VTEventHandler {
	...
	function handleEvent($eventName, $entityData) {
		...
		if (!($entityData->focus instanceof HelpDesk)) {
			return;
		}
		...
		if($eventName == 'vtiger.entity.beforesave') {
			...
			$log->debug("handleEvent HelpDeskHandler vtiger.entity.beforesave entered");
			global $adb;
			$id = $entityData->getId();
			...
			if( !$entityData->isNew() )
			{
				$log->debug("handleEvent HelpDeskHandler vtiger.entity.beforesave this is an update");
				$sql = "SELECT 
				...
				while ($row = $adb->fetchByAssoc($res)) {
					...
					$projecttype = vtlib_purify($row['projecttype']);
				}
				$focus->column_fields['project_id'] = $project_id;
				$focus->column_fields['projecttype'] = $projecttype;
			}
			$log->debug("handleEvent HelpDeskHandler vtiger.entity.beforesave treminated");
		}
		
		if($eventName == 'vtiger.entity.aftersave') {
			$log->debug("handleEvent HelpDeskHandler vtiger.entity.aftersave entered");
			$id = $entityData->getId();
			$module = $entityData->getModuleName();
			$focus = $entityData->focus;
			$log->debug("handleEvent HelpDeskHandler vtiger.entity.aftersave ticket title = ".$focus->column_fields['title']);
			if( !$entityData->isNew() )
			{
				$log->debug("handleEvent HelpDeskHandler vtiger.entity.aftersave this is an update");
			} elseif( $entityData->isNew()) {
				$log->debug("handleEvent HelpDeskHandler vtiger.entity.aftersave this is an insert");
				$log->debug("handleEvent HelpDeskHandler vtiger.entity.aftersave this is an insert focus->id = ". $focus->id );
			}
			$log->debug("handleEvent HelpDeskHandler vtiger.entity.aftersave terminated");
		}
	}
}
