...
// danzi.tn@20150102 aggiunta progetto in creazione ticket da modulo Progetti 
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] =='Project' && isset($_REQUEST['parent_id']) && $_REQUEST['parent_id'] > 0) {
	...
	$sql="SELECT projectid, projectname, project_no, linktoaccountscontacts, projecttype FROM vtiger_project WHERE vtiger_project.projectid = ?";
	$result = $adb->pquery($sql,array($parent_id ));
	$account_id = $adb->query_result($result,0,"linktoaccountscontacts");
	$projecttype = $adb->query_result($result,0,"projecttype");
	$_REQUEST['account_id'] = $account_id;
	$_REQUEST['parent_id'] = $account_id;
	$_REQUEST['project_id'] = $parent_id;
	$_REQUEST['projecttype'] = $projecttype;
}
// danzi.tn@20150102e parent_id 
